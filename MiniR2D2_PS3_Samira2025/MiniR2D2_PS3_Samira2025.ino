...
float dome_speed = 1;  //0.75 Aumenta o disminuye para controlar velocidad domo, el valor máximo es 1,0
...
#define frontarm1_MAX 370   //370
#define frontarm2_MIN 340   //340 L2
#define frontarm2_MAX 540   //540

// Define los valores mínimos y máximos para los servos
#define LIFTUP 420   // 420  +Nº mas alto, -Nº mas bajo para arriba
#define LIFTDOWN 230 // 236  +Nº mas alto, -Nº menos alto para abajo
#define TILTMIN 443  // 443 Valor para para ponerse recto +Nº mas recto, -Nº menos recto
#define TILTMAX 250  // 260 Valor para para dar inclinacion +Nº menos inclinacion -Nº mas inclinacion
...
#define periscopio_MIN 404  // 404
#define periscopio_MAX 214  // 214
...
// --- PATCH RETENCIÓN TILT ---
int currentTiltPosition = TILTMAX; // Guarda la posición actual del tilt
// --- FIN PATCH RETENCIÓN TILT ---
...
////////HOLOS////////
#define HOLO1_MIN 50  // 100  Top
#define HOLO1_MAX 600 // 550
#define HOLO2_MIN 50  // 50 Frente
#define HOLO2_MAX 600 // 550
#define HOLO3_MIN 50  // 150 Trasero
#define HOLO3_MAX 600 // 550

bool moving[] = {false, false, false};
unsigned long lastMoveTime[] = {0, 0, 0};
const unsigned long moveIntervals[] = {2000, 2500, 3000}; // Intervalos de tiempo entre movimientos en milisegundos (3, 4 y 5 segundos)
...
void moveTilt(int targetPosition) {
  ...
  // --- PATCH RETENCIÓN TILT ---
  currentTiltPosition = targetPosition;
  // --- FIN PATCH RETENCIÓN TILT ---
}
...
void efecto12() {
  ...
  if (currentMillis - previousMillis >= interval12) {
    ...
    // Definir el color rosa
    CRGB rosa = CRGB(255, 0, 128);

    // Gradualmente cambia la intensidad del color rosa
    for (int j = 0; j < 256; j++) {
      for (int i = 0; i < NUM_LEDS; i++) {
        leds[i] = rosa; // Establecer el color rosa constante
        leds[i].fadeToBlackBy(255 - j); // Ajustar la intensidad gradualmente
      }
      FastLED.show(); // Mostrar los cambios en los LEDs
      // Pequeña pausa para la transición suave
      // No bloquea el programa, permite seguir ejecutando otras tareas
      if (currentMillis - previousMillis >= interval12) {
        break; // Salir del bucle si ha pasado el tiempo del intervalo
      }
    }

    Serial.println("efecto rosa");
  }
}
...
// Ajuste de la velocidad basado en los estados centreliftState y legtiltState
  if (centreliftState == CENTRE_LIFT_UP && legtiltState == LEGS_TO_CENTER) {
      ...
      speedX *= 0.55;
      speedY *= 0.55;
      speedZ *= 0.70;  //valor para giro domo en 2 piernas
  }
  ...
  float dome_factor = 1.0;
  if (centreliftState == CENTRE_LIFT_UP && legtiltState == LEGS_TO_CENTER) {
      }
  speedZ = constrain(speedZ, -255, 255);
  motorCPWM = speedZ *= 0.80;
...
//    Serial.println(frontarm2);
  ...
  if ( Ps3.event.button_down.circle ) {
    ...
    int randomNumber = random(1, 45); // Genera un número aleatorio 
  ...
  if ( Ps3.event.button_down.square ) {
    ...
    int randomNumber2 = random(46, 56); // Genera un número aleatorio 
  ...
  // Luces holos y paneles - Botón Derecho Cruzeta     
...
// Luces holos y paneles Rosa - Botón Izquierdo Cruzeta     
if (Ps3.event.button_down.left) {
  Serial.println("boton cruzeta izquierdo");
   if (efecto12Activo) {
      efecto12Activo = false; // Desactivar el efecto 12
      musicPlayer.play(42);
      // Apagar todas las luces
      fill_solid(leds, NUM_LEDS, CRGB::Black);
      FastLED.show();
    } else {
      efecto12Activo = true; // Activar el efecto 12
      musicPlayer.play(4);
      efecto12(); // Llamar a la función efecto12
    }
  } 
}
...
void loop() {
  ...
  // --- PATCH RETENCIÓN TILT ---
  // Refuerza periódicamente la posición del servo de inclinación
  pwm1.setPWM(4, 0, currentTiltPosition);
  // --- FIN PATCH RETENCIÓN TILT ---
